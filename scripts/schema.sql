...
CREATE TABLE cards (
    ...
    CONSTRAINT FK_cards_bank                            FOREIGN KEY (bank_id)
                                                            REFERENCES banks (id),
    CONSTRAINT CHK_cards_network    CHECK (card_network IN ('VISA','MASTERCARD','UNIONPAY','AMEX','OTHER') OR card_network IS NULL),
    CONSTRAINT CHK_cards_type       CHECK (card_type    IN ('DEBIT','CREDIT','PREPAID') OR card_type IS NULL)
);
GO
...
CREATE TABLE deals (
    ...
    powered_by          NVARCHAR(100)   NULL,
    source_like_count   INT             NOT NULL DEFAULT 0,
    created_at          DATETIME2       NOT NULL DEFAULT SYSUTCDATETIME(),
    updated_at          DATETIME2       NOT NULL DEFAULT SYSUTCDATETIME(),
    ...
    CONSTRAINT CHK_deals_percentage_range   CHECK (percentage_value IS NULL
...
CREATE TABLE transaction_imports (
    ...
    CONSTRAINT FK_transaction_imports_user      FOREIGN KEY (user_id)
                                                    REFERENCES users (id),
    CONSTRAINT CHK_transaction_imports_status   CHECK (status IN ('PENDING','PROCESSING','COMPLETED','FAILED','PARTIAL'))
);
GO
...
CREATE TABLE transactions (
    ...
    CONSTRAINT FK_transactions_category     FOREIGN KEY (category_id)
                                                REFERENCES categories (id),
    CONSTRAINT CHK_transactions_confidence  CHECK (match_confidence IS NULL
...
CREATE INDEX IX_transactions_restaurant_id      ON transactions (restaurant_id) WHERE restaurant_id IS NOT NULL;
CREATE INDEX IX_transactions_category_id        ON transactions (category_id)   WHERE category_id IS NOT NULL;
CREATE INDEX IX_transactions_transaction_date   ON transactions (transaction_date);
CREATE INDEX IX_transactions_import_id          ON transactions (import_id)     WHERE import_id IS NOT NULL;
...
CREATE INDEX IX_user_preferences_category_id    ON user_preferences (category_id)   WHERE category_id IS NOT NULL;
CREATE INDEX IX_user_preferences_restaurant_id  ON user_preferences (restaurant_id) WHERE restaurant_id IS NOT NULL;
GO
...
CREATE TABLE merchant_mappings (
    ...
    CONSTRAINT CHK_merchant_mappings_confidence CHECK (confidence IS NULL
