...
# categorization_engine/transform.py (Update just this dictionary)

CATEGORY_LABELS = {
    "salary": "Salary",
    "profit_income": "Profit / Refunds",
    # CHANGE THIS LINE SO IT MATCHES YOUR DATABASE DEALS
...
def generate_pdf_report(df: pd.DataFrame) -> io.BytesIO:
    ...
    with PdfPages(buf) as pdf:
        ...
        header_ax.text(
            0.5,
            0.22,
            "Categorization Engine  ·  Spending Report",
            ha="center",
            va="center",
            color=FG_TEXT,
            fontsize=14,
        ...
        stat_labels = [
            ...
            (
                "Biggest Spend",
                f"PKR {cat_summary['total'].iloc[0]:,.0f}" if len(cat_summary) else "—",
            ),
        ]
        ...
        fig.text(
            0.5,
            0.935,
            "Proportion of total spending by category",
            ha="center",
            va="top",
            color=ACCENT,
            fontsize=11,
        ...
        if "date" in spend_debit.columns and spend_debit["date"].notna().any():
            monthly = spend_debit.set_index("date")["debit"].resample("ME").sum()
            ax.fill_between(monthly.index, monthly.values, alpha=0.25, color=ACCENT)
            ax.plot(
                monthly.index,
                monthly.values,
                color=ACCENT,
            ...
            ax.text(
                0.5,
                0.5,
                "Date column not available for trend analysis",
                ha="center",
                va="center",
                color=FG_TEXT,
                fontsize=13,
        ...
        bar_colors = [PIE_COLORS[i % len(PIE_COLORS)] for i in range(len(sorted_count))]
        ...
        ax.set_xlabel("Number of Transactions", color=FG_TEXT, fontsize=11, labelpad=10)
        ax.xaxis.label.set_color(FG_TEXT)
        ax.tick_params(axis="x", colors=FG_TEXT)
        ax.tick_params(axis="y", colors=FG_TEXT, labelsize=11)
        ax.grid(axis="x", color=GRID_CLR, linewidth=0.8, linestyle="--")
